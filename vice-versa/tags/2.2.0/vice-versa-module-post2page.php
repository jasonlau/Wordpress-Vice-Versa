...
<div class="viceversa-debug-container"><form id="viceversa-debug-form" method="get"> <label>Test Mode:</label> <input type="checkbox" name="viceversa_debug" class="viceversa-debug" title="Run this module in test mode. This allows you to run test conversions." value="true"<?php
	if(VICEVERSA_DEBUG) echo ' checked="checked"';
?> /><?php
	if(VICEVERSA_DEBUG){echo ' <span class="debug-on">Test Mode Is On</span>';} 
?><input type="hidden" name="page" value="<?php echo $_REQUEST['page'] ?>" />
            <input type="hidden" name="paged" value="<?php echo $_REQUEST['paged'] ?>" />
            <input class="viceversa-mode" type="hidden" name="viceversa_mode" value="<?php echo $_REQUEST['viceversa_mode'] ?>" />
            <input type="hidden" id="per-page-hidden" name="per_page" value="<?php echo $per_page ?>" /><input type="hidden" name="viceversa_debug" value="<?php echo $_REQUEST['viceversa_debug'] ?>" /></form></div>
<br />
...
jQuery.cookie = function(name, value, options) {
    ...
    jQuery(function($){
	   ...
	   $("div.actions").first().prepend('<?php
        ...
        ?> <input class="vv-help button-secondary hidden" type="button" value="?" title="Info" /><h3>Convert A Post To A Page</h3><br /><strong><?php _e('Items Per Page', 'vice-versa') ?>:</strong> <input type="text" id="per-page" size="4" value="<?php echo $per_page ?>" /><input class="vv-go button-secondary action" type="submit" value="<?php _e('Go', 'vice-versa') ?>" />');
    ...
    $("#viceversa-list-table-form").append('<fieldset class="viceversa-bulk-assign-field viceversa-info"><legend style="padding: 0px 4px"><strong><?php _e('Bulk Parent Selector', 'vice-versa') ?></strong></legend><div id="viceversa-bulk-assign">' + $("#viceversa-assign-to").html() + '</div></fieldset>');
